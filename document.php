<?
  # $Id: document.php,v 1.50 2001/12/01 16:17:18 sven Exp $
  #
  # Document lookup.
  #
  ...
  unset ($path_tail);		# Contains tail of path that wasn't analysed.
  				# This is used to pass extra arguments.
  unset ($document_template);	# Hold intital document template.
  ...
  function document_process ($root_table, $root_id, $root_template)
  {
    ...
    list ($dirtype, $table, $id, $vdir, $path_tail, $name)
      = document_path_to_directory (
	explode ('/', $PATH_INFO), $root_table, $root_id
      );
    ...
    if (sizeof ($path_tail) > 0) {
      $res =& $db->select (
        'id', 'obj_classes', 'name=\'' . $path_tail[0] . '\''
      );
      if ($res->num_rows () > 0)
    ...
    # Would be worth to move this to a stand-alone script.
    if ($name == 'OBJ') {
