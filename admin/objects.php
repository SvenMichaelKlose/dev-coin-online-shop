<?
  # $Id: objects.php,v 1.21 2001/12/01 16:31:46 sven Exp $
  #
  # Object editor.
  #
  ...
  #	Creates an object and stores the object-id in the referenced
  #	table.
  ...
  function remove_object (&$this)
  {
    ...
    $this->ui->confirm ($q,
                        $lang['yes'], 'remove_object4real', $arg,
			$lang['no'], 'edit_data', $arg);
  }
  ...
  function edit_data_navigator (&$this)
  {
	...
	echo '<td><b>' . $lang['current'] . ' --></b></td><td>-</td>';
        ...
        echo '<td>&nbsp;</td><td>';
	$p->link (
	  $lang['copy to current'],
	  'copy_object',
	  $this->arg_set_next (array ('class' => $class,
	  	 'table' => $otable,
	  	 'id' => $oid,
	...
	echo '</td>';
      ...
      echo '<td>';
      $p->link (
        $this->db->traverse_refs_from ($this, $t, $i, 'nav_linkpath', 1, false),
	'edit_data', array ('table' => $t, 'id' => $i, 'class' => $class)
      );
      echo '</td></tr>';
  ...
  function edit_data (&$this)
  {
    ...
    $p->radiobox (
      'is_local', $lang['yes'], $lang['no'], $lang['local'] . '<BR>'
    );
    # Radiobox public/private.
    $p->radiobox (
      'is_public', $lang['yes'], $lang['no'], $lang['public'] . '<BR>'
    );
	...
	if ($lines > 25)
      ...
      if ($mime == 'image')
        ...
        $p->label(
	  '<a href="' .
	  $p->filelink ('obj_data', 'data', $mime, $id) .
	  '">' . $lang['download'] . '</a>'
	);
    }
    $p->close_row ();
  ...
  function _object_box (&$this, $table, $id, $caller, $only_local = false)
  {
    ...
    while (list ($id_class, $class, $descr) = $res->fetch_array ()) {
      ...
      if (!isset ($cache[$id_class][0]) && ((!$only_local)
          || ($only_local && substr ($class, 0, 2) == 'u_'))) {
        $tmp = '[' . $p->_looselink (
               '<FONT COLOR="BLACK">' . $descr . '</FONT>' ,
	       'assoc_object',
	       $this->arg_set_next (
	         array ('table' => $table, 'id' => $id, 'class' => $class),
		 'edit_data',
		 $this->arg_set_caller (
	           array ('table' => $table, 'id' => $id, 'class' => $class,
		          'otable' => $table, 'oid' => $id)
	...
	if (isset ($cms_object_views[$class]))
	  ...
	  # Print image box
		...
		$this->arg_set_caller (
	      ...
	      $p->filelink (
		'obj_data', 'data', $obj['mime'], $obj['id'], $obj['data']
	      ) . '" alt="' . $imagename . '"></a><br>' .
	      '<FONT COLOR="' . $color . '">' . $descr . ', ' . $imagename .
	      '</FONT>' . $stat .
	      '</td></tr>' .
	      '</table></td>' . "\n";
	    continue;
        ...
        default:
            $tmp = '[' . $p->_looselink (
              '<FONT COLOR="' . $color . '">' . $descr . '</FONT>' . $stat,
	      'edit_data',
	      $this->arg_set_caller (
	        array ('table' => $table, 'id' => $id, 'class' => $class,
	               'otable' => $table, 'oid' => $id)
    ...
    if ($images) {
      if (!$only_local)
        echo '<tr><td bgcolor="#dddddd"><b>' . $lang['images'] .
	     ':</b></td></tr>'. "\n";
      echo '<tr><td align="center">' .
	   '<table border="0"><tr>' . $images . '</tr></table>' .
      	   '</td></tr>'. "\n";
    }
    if ($documents)
      echo '<tr><td bgcolor="#dddddd"><b>' . $lang['documents'] .
           ':</b></td></tr><tr><td>' . $documents . "</td></tr>\n";
    if ($user_defined)
      echo '<tr><td bgcolor="#dddddd"><b>' . $lang['user defined classes'] .
           ':</b></td></tr><tr><td>' . $user_defined . "</td></tr>\n";
    if ($configuration)
      echo '<tr><td bgcolor="#dddddd"><b>' . $lang['configuration'] .
           ':</b></td></tr><tr><td>' . $configuration . "</td></tr>\n";
    echo '</table>';
