<?
  ...
  $current_index = -1;          # Initialized by the url scanner at end of
  $current_indices = '';        # file and by parse_result_set ().
  $url_vars = array ();
  ...
  function cms_create_context ($dirtype, $table = '', $id = 0)
  {
    ...
    if ($table && $id && isset ($scanner->tables[$dirtype]) &&
        $scanner->tables[$dirtype] == $table)
      return;
...
$scanner->context_table = $dirtype;
  ...
  fuNction tag_link ($attr, $use_key = true)
  {
    ...
    if ($relative_index)
      if (isset ($list_offsets[$listsource]))
        $url_vars['list_offsets[' . $listsource . ']']
	  = $list_offsets[$listsource] + $relative_index;
      else
  ...
  function tag_num_type ($attr)
  {
    ...
    if ($apath[sizeof ($apath) - 1] != $srctab)
      return '<!-- cms: No subdirectories of type ' . $arg .
             ' in directories of type ' . $scanner->dirtypes[$srctab] . '.';
