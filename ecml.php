<?
  # $Id: ecml.php,v 1.22 2001/10/23 17:32:28 sven Exp $
  #
  # ECML extension for dev/con cms.
  #
  ...
  function ecml_address_field ($arg, $address)
  {
    ...
    $res = $db->select (
      'id_address_' . $address, 'ecml_order',
      'id_session=' . $GLOBALS['session']->id ()
    );
    if ($res->num_rows () < 1)
      return;
  ...
  function ecml_add_address ($db, $type, $idname)
  {
    ...
    if ($set) {
      ...
      if (!$id) {
        ...
        $id = $db->insert_id ();
        $db->update ('ecml_order',
                     $idname . '=' . $id, 'id_session=' . $SESSION_ID);
      }
  ...
  function ecml_parse_form ()
  {
    ...
    # Read in ECML-Fields.
    if (isset ($GLOBALS['Ecom_SchemaVersion'])
        && $GLOBALS['Ecom_SchemaVersion'] == 'http://www.ecml.org/version/1.1') {
      global $order_errors;
	...
	$res = $db->select ('id_address_' . $addrtype,
                            'ecml_order', 'id_session=' . $SESSION_ID);
